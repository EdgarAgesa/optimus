...
function getDeals() {
  ...
  categories.forEach(cat => {
    ...
    if (pool.length > 0) {
      const pick = pool.find(p => !deals.find(d => d.sku === p.sku));
      if (pick) deals.push(pick);
    }
  });
            ...
            >
              ...
              <div className="deal-info">
                ...
                <div className="deal-pricerow">
                  ...
                  {p.oldPrice && (
                    <span className="deal-oldprice">{p.oldPrice}</span>
                  )}
                </div>
              </div>
